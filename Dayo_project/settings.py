...
# Logging — Django's default gates the console handler behind DEBUG=True, so
# with DEBUG=False (production) unhandled 500 tracebacks are silently dropped
# and never reach Render's logs. This always writes errors to stderr so real
# failures (DB connection errors, etc.) are visible in the Render Logs tab.
LOGGING = {
    'version': 1,
